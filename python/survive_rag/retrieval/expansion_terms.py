"""Domain vocabulary bridging how Indian users phrase an emergency and how
the guides word it.

Three kinds of entry:

1. **English synonym bridges** -- ``bleeding`` -> ``hemorrhage tourniquet``.
2. **Hindi / Hinglish transliterations** -- a person in a panic types
   ``"khoon nikal raha hai"``, not ``"haemorrhage"``. This is the highest-value
   part of the table: without it a large share of real Indian queries retrieve
   nothing at all.
3. **India-specific nouns** -- LPG, lathi, nala, cloudburst, ORS, ASV.

Mirrors ``lib/utils/expansion_terms.dart``; ``tests/test_expansion.py``
...
EXPANSION_TERMS: dict[str, tuple[str, ...]] = {
    ...
    "unconscious": ("recovery", "position", "airway", "unresponsive", "breathing"),
    ...
    "disabled": ("disability", "wheelchair", "accessible", "assistive", "evacuate"),
